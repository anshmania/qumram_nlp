...
f = configNLP.qumram_lexicon_path
...
#    WORD_VALENCE_DICT = make_lex_dict(f)
#except:
#    f = os.path.join(os.path.dirname(__file__),'Users/anshmania/Anaconda2/Lib/site-packages/nltk/sentiment/qumram_lexicon.txt')
#    WORD_VALENCE_DICT = make_lex_dict(f)
...
BOOSTER_DICT = {"absolutely": B_INCR, "amazingly": B_INCR, "awfully": B_INCR, "completely": B_INCR, "considerably": B_INCR,
                ...
                "very": B_INCR,"GBP":B_INCR,"Euros":B_INCR,"Pounds":B_INCR,"$":B_INCR,
                "almost": B_DECR, "barely": B_DECR, "hardly": B_DECR, "just enough": B_DECR,
                "kind of": B_DECR, "kinda": B_DECR, "kindof": B_DECR, "kind-of": B_DECR,
                "less": B_DECR, "little": B_DECR, "marginally": B_DECR, "occasionally": B_DECR, "partly": B_DECR,
                "scarcely": B_DECR, "slightly": B_DECR, "somewhat": B_DECR,
                "sort of": B_DECR, "sorta": B_DECR, "sortof": B_DECR, "sort-of": B_DECR, "sorry":B_DECR}

# check for special case idioms using a sentiment-laden keyword known to SAGE
SPECIAL_CASE_IDIOMS = {"the shit": 3, "the bomb": 3, "bad ass": 1.5, "yeah right": -2,
                       "cut the mustard": 2, "kiss of death": -1.5, "hand to mouth": -2,"not supposed to":-3,"insurance policy":3,
                       "amazing offer":4,"one time offer":3,"exclusively for you":3, "rare offer":3, "rare":1,
                "take a peek":1, "good deal":3, "take care":0,"a great product":2,"we are introducing":1.5,"exclusive offer":3,"account details":4}
...
#        normScore = 1-1/(math.exp((score)))
#    else:
#        normScore = 1-1/(math.exp((score)))
#    return normScore

##using sigmoid function to normalize without needing to guess a maximum value for the score
#def normalize(score):
#    if score==0:
#        score=score-2
...
#    remove stopwords from [wordsAndEmoticons]
    ...
    stop_words = [str(word).strip() for word in english_stops]
    for word in wordsAndEmoticons:
    ...
    for item in wordsAndEmoticons:
        ...
        if item_lowercase in WORD_VALENCE_DICT:
            ...
            if i > 0 and wordsAndEmoticons[i-1].lower() not in WORD_VALENCE_DICT:
                ...
                v = v+s1
                if negated([wordsAndEmoticons[i-2]]): v = v*n_scalar
            ...
            if i > 2 and wordsAndEmoticons[i-3].lower() not in WORD_VALENCE_DICT:
                ...
                if wordsAndEmoticons[i-3] == "never" and \
                   ...
                   (wordsAndEmoticons[i-1] == "so" or wordsAndEmoticons[i-1] == "this"):
                    v = v*1.25
                # otherwise, check for negation/nullification
                elif negated([wordsAndEmoticons[i-6]]): v = v*n_scalar
    ...
    s = {"non-compliance" : round(compound, 4)}
    return s